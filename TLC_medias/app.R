...
### basado en https://github.com/ShinyEd/intro-stats

# Define UI --------------------------------------------------------------------

ui <- fluidPage(
  # Title ----
  titlePanel("Teorema del Límite Central para las Medias", windowTitle = "TLC medias"),
  
  sidebarLayout(
    sidebarPanel(
      wellPanel(
        # Seleccione una distribución ----
        radioButtons("dist", "Distribución principal de la población:",
                     c("Normal" = "rnorm",
                          "Uniforme" = "runif",
                          "Sesgo a la derecha" = "rlnorm",
                          "Sesgo a la izquierda" = "rbeta"),
                     selected = "rnorm"),
        # hr(),
...
server <- function(input, output, session) {
  ...
  output$pop.dist = renderPlot({
    ...
    distname = switch(input$dist,
                      ...
                      rlnorm = "Distribución de la población: Sesgo a la derecha",
                      rbeta = "Distribución de la población: Sesgo a la izquierda",
    ...
    if (input$dist == "rnorm"){
      ...
      ggplot(data = pop, aes(x = samples, y = ..density..)) + 
        geom_histogram(bins = 45, color = "white", fill = "#679BE0") +
        ...
        annotate("text", x = x_pos, y = y_pos,
                 label = paste("Media de x", "=", bquote(.(m_pop)),
                               "\n", "DE of x", "=", bquote(.(sd_pop))),
                 color = "black", size = 5) +
  ...
  output$sample.dist = renderPlot({
    ...
    for(i in 1:8){
      ...
      plots[[i]] = ggplot(x, aes_string(x = paste0("V", i))) +
        ...
        labs(title = paste("Muestra", i), x = "", y = "") +
        theme_light(base_size = 13) +
        annotate("text", x = x_pos, y = 1.8,
                 label = paste("x_barra", "=", bquote(.(mean)),
                               "\n", "S", "=", bquote(.(sd))),
                 color = "black", size = 3) +
  ...
  output$sampling.dist = renderPlot({
    ...
    distname = switch(input$dist,
                      ...
                      rlnorm  = "población con sesgo a la derecha",
                      rbeta = "población con sesgo a la izquierda",
    ...
    p = ggplot(data = ndist, aes(x = means, y = ..density..)) +
      geom_histogram(bins = 20, color = "white", fill = "#7AD1D6") +
      stat_density(geom = "line", color = "#009499", size = 1) +
      labs(title = paste("Distribución de las medias muestrales"),
           x = "Medias de cada muestra",
           y = "") +
      annotate("text", x = x_pos, y = y_pos,
               label = paste("Media de x_barra", "=", bquote(.(m_samp)),
                             "\n", "D.E. de x_bara", "=", bquote(.(sd_samp))),
               color = "black", size = 5) +
  ...
  output$sampling.descr = renderText({
    ...
    k = input$k
    n = input$n
    paste("*Distribución de las medias de", k, "musetras aleatorias,
          cada una de esas muestras consiste de", n, " observaciones
          de una ", distname)
    })
  ...
  output$CLT.descr = renderText({
    ...
    paste0("De acuerdo al Teorema del Límite Central (TLC), la *distribución de las medias muestrales* 
          de un número suficientemente grande de muestras, será parecida a una distribución normal.
          
          La media de la distribución de las medias muestrales será aproximadamente igual 
          a la media de la población (", m_pop, "), y la desviación estandar de las medias muestrales será
          aproximadamente igual a la desviación estándar de la población dividida para la raiz cuadrada
          del tamaño de la muestra (", s_pop,"/sqrt(",n, ") = ", se,").
          
          Abajo se puede ver el gráfico de la distribución de las medias muestrales. Para facilitar la
          comparación, a la derecha está el gráfico de la distribución de la población.")
